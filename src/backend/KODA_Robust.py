...
#print("test1111")
#print(tabulate(df_stop_times_rt[df_stop_times_rt["trip_id"]=='55700000078651044'], headers="keys",tablefmt="psql"))
...
#print(trip_speed_summary.head(20))

trip_speed_summary.to_csv("trip_speed_summary_2026_05_04.csv")
#print("trip summary 2")
#print(tabulate(trip_speed_summary[trip_speed_summary["trip_id"]=='55700000078906900'], headers="keys", showindex=False))
#print("trip summary 3")
...
df_base["_stop_sequence_sort"] = pd.to_numeric(
    df_base["stop_sequence"], errors="coerce"
)
...
first_stop_by_trip = (
    df_base.sort_values(["trip_id", "timestamp_dt", "_stop_sequence_sort"])
    .groupby("trip_id", as_index=False)
    .first()
)
last_stop_by_trip = (
...
last_arrive_by_trip = (
    df_base[df_base["activity"].str.startswith("arrive_stop", na=False)]
    .sort_values(["trip_id", "timestamp_dt", "_stop_sequence_sort"])
    .groupby("trip_id", as_index=False)
    .last()
)

trip_boundaries = first_stop_by_trip.merge(
    last_stop_by_trip[[
        "trip_id",
        "event_id",
        "activity",
...
trip_boundaries = trip_boundaries.merge(
    last_arrive_by_trip[[
        "trip_id",
        "timestamp",
        "timestamp_dt",
    ...
    on="trip_id",
    how="left",
    suffixes=("", "_last_arrive"),
)

for _, temp_table in trip_boundaries.groupby("_vehicle_route_fallback_group", dropna=False):
    ...
    for i in range(len(temp_table) - 1):
        ...
        current_activity = str(current_row["activity_last"])
        next_activity = str(next_row["activity_first"])
        is_trip_change = current_row["trip_id"] != next_row["trip_id"]
        is_departure_to_arrival = (
            current_activity.startswith("departure_stop")
            and next_activity.startswith("arrive_stop")
        )

        if not is_trip_change or not is_departure_to_arrival:
            continue

        current_date = current_row["timestamp_dt_last"]
        current_stop_id = current_row["stop_id_last"]
        current_stop_sequence = current_row["stop_sequence_last"]
        current_stop_name = current_row["stop_name_last"]
        current_stop_lat = current_row["stop_lat_last"]
        current_stop_lon = current_row["stop_lon_last"]
        current_direction_id = current_row["direction_id_last"]
        current_route_type = current_row["route_type_last"]
        current_route_short_name = current_row["route_short_name_last"]
        current_route_id = current_row["route_id_last"]
        current_service_id = current_row["service_id_last"]
        current_agency_id = current_row["agency_id_last"]
        current_organization_id = current_row["organization_id_last"]
        current_operator = current_row["organization_name_last"]
        next_date = next_row["timestamp_dt_first"]

        last_arrive_date = current_row["timestamp_dt"]
        use_last_arrive_boundary = (
            pd.notna(last_arrive_date)
            and current_date > next_date
            and last_arrive_date < current_date
        )

        if use_last_arrive_boundary:
            current_date = last_arrive_date
            current_stop_id = current_row["stop_id"]
            current_stop_sequence = current_row["stop_sequence"]
            current_stop_name = current_row["stop_name"]
            current_stop_lat = current_row["stop_lat"]
            current_stop_lon = current_row["stop_lon"]
            current_direction_id = current_row["direction_id"]
            current_route_type = current_row["route_type"]
            current_route_short_name = current_row["route_short_name"]
            current_route_id = current_row["route_id"]
            current_service_id = current_row["service_id"]
            current_agency_id = current_row["agency_id"]
            current_organization_id = current_row["organization_id"]
            current_operator = current_row["organization_name"]

        transition_distance_m = haversine_distance_m(
            current_stop_lat,
            current_stop_lon,
            next_row["stop_lat_first"],
            next_row["stop_lon_first"],
        )
        is_stationary_transition = (
            pd.notna(transition_distance_m)
            and transition_distance_m == 0
        )
        transition_activity = (
            "stationary_pause"
            if is_stationary_transition
        ...
        a_event_rows.append({
            ...
            "timestamp": next_row["timestamp_first"],
            "stop_id": next_row["stop_id_first"],
            "stop_sequence": next_row["stop_sequence_first"],
            "stop_name": next_row["stop_name_first"],
            "stop_lat": next_row["stop_lat_first"],
            "stop_lon": next_row["stop_lon_first"],
            "old_stop_lat": current_stop_lat,
            "old_stop_lon": current_stop_lon,
            "direction_id": next_row["direction_id_first"],
            "vehicle_id": current_row["vehicle_id_first"],
            "route_type": current_row["route_type_first"],
            "route_short_name": next_row["route_short_name_first"],
            "route_id": next_row["route_id_first"],
            "service_id": next_row["service_id_first"],
            "agency_id": next_row["agency_id_first"],
            "organization_id": current_row["organization_id_first"],
            "operator": current_row["organization_name_first"]
        })

    if not temp_table.empty:
        last_row = temp_table.iloc[-1]
        end_date = last_row["timestamp_dt_last"]
        end_stop_id = last_row["stop_id_last"]
        end_stop_sequence = last_row["stop_sequence_last"]
        end_stop_name = last_row["stop_name_last"]
        end_direction_id = last_row["direction_id_last"]
        end_route_type = last_row["route_type_last"]
        end_route_short_name = last_row["route_short_name_last"]
        end_route_id = last_row["route_id_last"]
        end_service_id = last_row["service_id_last"]
        end_agency_id = last_row["agency_id_last"]
        end_organization_id = last_row["organization_id_last"]
        end_operator = last_row["organization_name_last"]

        a_event_rows.append({
            "old_trip_id": last_row["trip_id"],
            "trip_id": pd.NA,
            "activity": "end_shift",
            "timestamp": end_date + pd.Timedelta(microseconds=1),
            "stop_id": end_stop_id,
            "stop_sequence": end_stop_sequence,
            "stop_name": end_stop_name,
            "direction_id": end_direction_id,
            "vehicle_id": last_row["vehicle_id_first"],
            "route_type": end_route_type,
            "route_short_name": end_route_short_name,
            "route_id": end_route_id,
            "service_id": end_service_id,
            "agency_id": end_agency_id,
            "organization_id": end_organization_id,
...
for _, temp_table in trip_boundaries.groupby("_vehicle_route_fallback_group", dropna=False):
    ...
    current_row = temp_table.iloc[0]
    current_date = current_row["timestamp_dt_first"]

    # begin_shift gibt es nur, wenn fuer diese Fahrzeuggruppe vorher keine Aktivitaet existiert.
    begin_shift_rows.append({
        ...
        "stop_lat": current_row["stop_lat_first"],
        "stop_lon": current_row["stop_lon_first"],
...
def normalize_trip_transition_boundaries(transition_events, begin_events, event_table):
    ...
    pause_activities = {"stationary_pause", "repositioning_pause"}
    ...
    if not transition_events.empty:
        ...
        transition_events["_activity_priority"] = transition_events["activity"].map(
            {"stationary_pause": 0, "repositioning_pause": 0, "end_shift": 1}
        ).fillna(9)
        ...
        for row in transition_events.itertuples():
            start_trip = (
                str(row.trip_id)
                if row.activity in pause_activities and pd.notna(row.trip_id)
                else None
            )
    ...
    used_start_trips = set(
        transition_events.loc[
            transition_events["activity"].isin(pause_activities)
    ...
    if not missing_end_events.empty:
        fallback_end_shift_events = pd.DataFrame({
            "trip_id": pd.NA,
            ...
            "stop_lat": missing_end_events["stop_lat"],
            "stop_lon": missing_end_events["stop_lon"],
            "old_stop_lat": pd.NA,
            "old_stop_lon": pd.NA,
        ...
        fallback_end_shift_events["event_id"] = pd.NA
        transition_events = pd.concat(
            [transition_events, fallback_end_shift_events[transition_columns]],
            ignore_index=True,
        )
...
#print("stationary_pause:", (a_events["activity"] == "stationary_pause").sum())
#print("repositioning_pause:", (a_events["activity"] == "repositioning_pause").sum())
#print("end_shift:", (a_events["activity"] == "end_shift").sum())
...
"""
Aux_e2o Aktivitaet A: change direction/position pause

"""

pause_activity_names = ["stationary_pause", "repositioning_pause"]
activity_a_events = a_events[a_events["activity"].isin(pause_activity_names)].copy()
...
"""
Aktivitaet B: end_shift
"""
activity_b_events = a_events[a_events["activity"] == "end_shift"].copy()
...
route_change_attributes = [
    {"name": "route_type", "type": "integer"},
    {"name": "old_stop_lat", "type": "float"},
    {"name": "old_stop_lon", "type": "float"},
]
...
event_type_records.extend([
    ...
    {"name": "stationary_pause", "attributes": route_change_attributes},
    {"name": "repositioning_pause", "attributes": route_change_attributes},
    {"name": "end_shift", "attributes": route_change_attributes},
])

eventTypes = pd.DataFrame(event_type_records)
#print(aux_event_log["shape_dist_traveled"])

for column in ["old_stop_lat", "old_stop_lon"]:
    if column not in aux_event_log.columns:
        aux_event_log[column] = pd.NA
...
events = pd.DataFrame([ {"id": e.event_id,
                        ...
                        "time": e.timestamp,
                         "attributes": [
                            ...
                            {
                            "name": "old_stop_lat",
                            "value": e.old_stop_lat
                            },
                            {
                            "name": "old_stop_lon",
                            "value": e.old_stop_lon
                            },
                            {
                            "name": "stop_sequence",
...
with open("ocel_koda_2026_05_04_w_stop_names_no_layover_shift_position_work.json", "w", encoding="utf-8") as f:
