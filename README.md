...
- `Event Builder` uses `KODA.py` and derives the activity types `begin_shift`, `parking`, `arrive_stop`, `departure_stop`, and `direction_change`. `parking` and `direction_change` are mainly derived from the temporal distance between the current trip and the next trip. `direction_change` represents a transition without relevant vehicle movement. `parking` represents vehicle movement without passengers, for example a movement from a stop to a depot.
- `Event Builder+` uses `KODA_Robust.py` and uses a more robust transition logic between two trips. Instead of relying only on the temporal distance, it checks whether the GPS locations indicate movement between the end of one trip and the start of the next trip. If movement is detected, the transition is classified as `repositioning_pause`. If no relevant movement is detected, it is classified as `stationary_pause`. Both describe time without passengers between trips.
