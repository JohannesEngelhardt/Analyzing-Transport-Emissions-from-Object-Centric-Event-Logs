...
"""
df_stops = pd.read_csv('/Users/johannesengelhardt/PycharmProjects/Masterthesis3/idfm-gtfs_french/stops.txt', sep=",")
df_stop_times = pd.read_csv('/Users/johannesengelhardt/PycharmProjects/Masterthesis3/idfm-gtfs_french/stop_times.txt', sep=",")
df_routes = pd.read_csv('/Users/johannesengelhardt/PycharmProjects/Masterthesis3/idfm-gtfs_french/routes.txt',  encoding="latin1", sep=",")
df_agency = pd.read_csv('/Users/johannesengelhardt/PycharmProjects/Masterthesis3/idfm-gtfs_french/agency.txt', sep=",")
df_calendar_dates = pd.read_csv('/Users/johannesengelhardt/PycharmProjects/Masterthesis3/idfm-gtfs_french/calendar_dates.txt', sep=",")
df_trips = pd.read_csv('/Users/johannesengelhardt/PycharmProjects/Masterthesis3/idfm-gtfs_french/trips.txt', sep=",")
df_calender_expanded = pd.read_csv('/Users/johannesengelhardt/PycharmProjects/Masterthesis3/calendar_expanded.csv')

df_calendar_dates["date"] = pd.to_datetime(df_calendar_dates["date"], format="%Y%m%d")
df_calendar_dates = df_calendar_dates.drop(columns=["exception_type"])
df_calendar_dates = df_calendar_dates.drop_duplicates()


df_stops = df_stops.drop(columns=["zone_id"])
...
print(tabulate(event_log_arrival[event_log_arrival["trip_id"]=='IDFM:TRANSDEV_EXPRESS_ROISSY:239836-C01298-21232811'], headers="keys", tablefmt="psql"))
...
print(tabulate(event_log_departure[event_log_departure["trip_id"]=='IDFM:TRANSDEV_EXPRESS_ROISSY:239836-C01298-21232811'], headers="keys", tablefmt="psql"))

#print(tabulate(event_log_arrival.head(100), headers="keys", tablefmt="psql", showindex=False))
...
#print(tabulate(event_log_departure.head(100), headers="keys", tablefmt="psql", showindex=False))
...
#event_log_departure= event_log_departure[["trip_id","activity","departure_time", "stop_id", "stop_sequence", "stop_name","stop_lat", "stop_lon", "start_pickup_drop_off_window",
 #                                         "end_pickup_drop_off_window", "pickup_type", "drop_off_type", "local_zone_id", "stop_headsign", "timepoint", "pickup_booking_rule_id",
   #                                                                                                                                                    "drop_off_booking_rule_id",
  #                                        "stop_code", "location_type", "parent_station", "stop_timezone", "level_id", "wheelchair_boarding", "platform_code", "stop_access"]]
...
event_log = event_log.sort_values(["timestamp", "activity"]).reset_index(drop=True)

#print(tabulate(event_log.head(100), headers="keys", tablefmt="psql"))
...
aux_event_log = event_log.merge(df_trips, on="trip_id", how="left").drop_duplicates()
print(tabulate(aux_event_log[aux_event_log["trip_id"]=='IDFM:TRANSDEV_EXPRESS_ROISSY:239836-C01298-21232811'], headers="keys", tablefmt="psql"))




aux_event_log = aux_event_log.merge(df_routes, on="route_id", how="left").drop_duplicates()
print(tabulate(aux_event_log[aux_event_log["trip_id"]=='IDFM:TRANSDEV_EXPRESS_ROISSY:239836-C01298-21232811'], headers="keys", tablefmt="psql"))
...
#calender_expanded = pd.concat([df_calender_expanded, df_calendar_dates], axis=0, join="inner", ignore_index=True)
calender_expanded = pd.concat(
    [df_calender_expanded, df_calendar_dates],
    ignore_index=True
)
...
aux_event_log = aux_event_log.merge(df_agency, on="agency_id", how="left").drop_duplicates()
print(tabulate(aux_event_log[aux_event_log["trip_id"]=='IDFM:TRANSDEV_EXPRESS_ROISSY:239836-C01298-21232811'], headers="keys", tablefmt="psql"))

aux_event_log = aux_event_log.merge(calender_expanded, on="service_id", how="left")
print(tabulate(aux_event_log[aux_event_log["trip_id"]=='IDFM:TRANSDEV_EXPRESS_ROISSY:239836-C01298-21232811'], headers="keys", tablefmt="psql"))


aux_event_log["timestamp_hourly"] = aux_event_log["timestamp"]

aux_event_log["date"] = pd.to_datetime(aux_event_log["date"])

aux_event_log["timestamp"] = (
    aux_event_log["date"] +
    pd.to_timedelta(aux_event_log["timestamp_hourly"])
)


#aux_event_log = aux_event_log.drop_duplicates()

#aux_event_log["timestamp"] = aux_event_log["date"].dt.strftime("%Y-%m-%d") + ' ' + aux_event_log["timestamp_hourly"]

#print(tabulate(aux_event_log.head(1000), headers="keys", tablefmt="psql", showindex=False))
...
print(tabulate(aux_event_log[aux_event_log["activity"]=='departure_stop'].head(100), headers="keys", tablefmt="psql", showindex=False))
"""
...
#print(tabulate(aux_event_log[(aux_event_log["trip_id"]=='IDFM:RDBOUCLESDEMARNE:243818-C01274-46652513')& (aux_event_l#og["date"]=='2026-04-26 00:00:00')].head(1000).sort_values(["stop_sequence", "timestamp"]), headers="keys", tablefmt="psql", showindex=False))
print(tabulate(aux_event_log[(aux_event_log["date"]=='2026-04-26 00:00:00') & (aux_event_log["route_short_name"]=='303') & (aux_event_log["direction_id"]==0)].head(200).sort_values(["stop_sequence", "timestamp"]), headers="keys", tablefmt="psql", showindex=False))
print(tabulate(aux_event_log[(aux_event_log["date"]=='2026-04-26 00:00:00') & (aux_event_log["route_short_name"]=='303') & (aux_event_log["direction_id"]==1)].head(200).sort_values(["stop_sequence", "timestamp"]), headers="keys", tablefmt="psql", showindex=False))
...
cols = [
    ...
    "stop_name", "stop_lat", "stop_lon", "direction_id",
     "route_type", "route_short_name", "route_id", "service_id", "agency_id"
]
...
events_complete["time_sort"] = pd.to_datetime(events_complete["time"])
events_complete = events_complete.sort_values("time_sort").drop(columns=["time_sort"]).reset_index(drop=True)
...
with open("ocel3.json", "w", encoding="utf-8") as f:
